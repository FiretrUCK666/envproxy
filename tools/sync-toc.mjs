#!/usr/bin/env node
/*
 * 仓库内副本：本文件是 project-forge skill 的 scripts/sync-toc.mjs 的逐字副本。
 * 放进仓库是为了让 CI 每次提交都能校验「目录与标题一致」——目录是派生内容，
 * 手工维护一定会漂移，而锚点写错的表现只是「点了没反应」，没人会去点自己的目录。
 *   - 本仓库里的路径是 tools/sync-toc.mjs（上面文档里的 scripts/ 是 skill 侧的路径）。
 *   - skill 升级时整体覆盖本文件即可；不要在这里单独改，改动请回到 skill 侧再同步。
 */
/**
