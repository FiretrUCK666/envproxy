...
> Later, to update or fix anything: just **double-click `1-安装.cmd` again**. It's the "universal fix button" — no double install, no network break, nothing lost.
...
| shows "variables deleted" while VPN app is connected | nodes temporarily can't reach the **check endpoints' networks** (e.g. Google main hit by DNS pollution/routing issues) while all endpoints were in that network → false "down" | new versions expanded endpoints to 7 cross-network ones (fast lane + parallel fallback), this is fixed at the root; on an old version, double-click "install" once to update |
...
| VPN app moved into a very deep directory (D:\a\b\c\d\e\f) | auto-search doesn't cover that depth | double-click "install" once |
| want to go back to before-install | — | double-click "3-一键恢复", keep-or-delete log chosen on the spot |
...
> The Mac edition is a parallel file set (`envproxy.sh` / `locator.sh` / four `.command` etc.), see section 12.4.
...
- Only rule: **don't double-click "1-安装.cmd" inside the backup** (clicking it promotes the backup to the live install)
...
To update later: `git pull` for the latest, then double-click `1-安装.cmd` once (universal fix: loads new code and corrects variables).

> Encoding note: `envproxy.ps1` / logs use **UTF-8 BOM** so PowerShell 5.1 reads them correctly on **any Chinese/English Windows** (without BOM, Chinese systems read GBK and English systems Latin1 — both can cause syntax errors or mojibake). **If you ever edit `envproxy.ps1`, save it as UTF-8 with BOM** (VS Code / Notepad++ / Notepad all offer "UTF-8 with BOM"), then double-click "install" once so the monitor loads the new code.
...
3. **Moved into an extremely deep directory** (past search depth): double-click "install" once as fallback, everything else is automatic.
...
> Windows users can stop here. Below is only about Macs. The Windows set (four `.cmd` + `envproxy.ps1`) is untouched; the Mac set is parallel files — copy the whole folder to a Mac and it works.
...
2. Double-click **`1-安装.command`** (first time may need right-click → Open to pass Gatekeeper once, see 12.5)
3. Done when you see "monitor process started"; **open a new terminal** to verify: `env | grep -i proxy`

Install does the same three things: boot auto-start (LaunchAgent) + start monitor now + correct variables on the spot. Later, any weirdness: double-click `1-安装.command` once = universal fix.
...
Terminal users can also run: `bash install.sh` / `bash stop.sh` / `bash uninstall.sh` (add `--purge` to delete the log too, no questions) / `bash status.sh`. The `.command` files are just "double-click shells", all logic lives in the `.sh` files.

### 12.4 What the Mac set adds to the folder

```
EnvProxy/
  envproxy.sh          Mac core (mirrors envproxy.ps1, single file, stock commands only)
...
1. **Double-click does nothing / "cannot open"**: that's Gatekeeper. Three fixes, pick one: right-click → Open to allow once; or run `bash install.sh` in a terminal (always works); or `xattr -d com.apple.quarantine 1-安装.command` then double-click.
2. **Lost `chmod` bits** (common when copied from Windows): in a terminal, `cd` into the folder and run `chmod +x *.command *.sh` once.
...
Issue? Read section 6 troubleshooting first; still stuck → open an issue: `https://github.com/FiretrUCK666/envproxy/issues`. Include: your OS version, the full output of `4-查看状态`, recent relevant lines of `monitor/monitor.log`. To hack on the code, read `CONTRIBUTING.md` first.
