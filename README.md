...
Mac 版注入的也是整套 9 个变量（`HTTP_PROXY/http_proxy/HTTPS_PROXY/https_proxy/ALL_PROXY/all_proxy` 统一 `http://127.0.0.1:端口`，`NO_PROXY/no_proxy=localhost,127.0.0.1,::1`，`NODE_USE_ENV_PROXY=1`），逻辑参数与 Windows 一致：2 秒/3 秒轮询、双轮确认、连续 2 次失败才判断、7 端点快路+并行兜底、15 秒节流、日志 200KB 上限、每 30 轮（约 60–90 秒）自启自愈。**"2-停止监控"的语义也与 Windows 一致：停掉就真的停掉，直到下次登录。**
