...
### 它注入哪些变量（2026-09 起 ALL_PROXY 统一 http:// 写法）
...
> 以后想更新版本/修复任何问题：直接**再双击一次 `1-安装.cmd`** 就行，它是"万能修复按钮"，不会重复安装、不会断网、不会丢东西。
...
| **4-查看状态** | 显示监控/自启动/翻墙状态/变量/最近日志 | — | 想看它工作得怎么样 |
...
| 明明连着翻墙软件却显示"变量已删除" | 节点对**验证端点所在的域段**暂时不通（如 Google 主域被 DNS 污染/线路问题），而验证端点全是该域段 → 误判"断" | 新版本已把验证端点扩为 7 个跨域段（快路优先 + 失败并行兜底），此问题根治；遇到旧版本请重新双击一次"安装"更新 |
...
| 翻墙软件换了极深的目录（D:\a\b\c\d\e\f） | 自动搜索覆盖不到 | 双击一次"安装" |
| 想彻底回到没装之前 | — | 双击"3-一键恢复"，日志留不留当场选 |
...
> Mac 版另有一套并排文件（`envproxy.sh` / `locator.sh` / 4 个 `.command` 等），见第十二节 12.4。
...
- 唯一注意：**不要在备份文件夹里双击"1-安装.cmd"**（点了就等于把备份切换为正式项目）
...
1. **纯 TUN 模式**的翻墙软件（不开本地 HTTP 端口）：TUN 本身已经全局接管（终端自动翻墙），本工具检测不到端口 → 不注入 → **这是正确行为**，不需要它。
2. **"断开连接"≠ 退出软件**：多数翻墙软件的"断开"按钮不停内核，端口还活着。本工具以"流量真相"为准：翻得出去就注入，翻不出去才删除。
3. **搬家到极深层目录**（超过搜索深度）：搜索不到时双击一次"安装"兜底，其余场景全自动。
...
> Windows 用户看到这里就可以停了。下面只讲苹果电脑。Windows 的 4 个 `.cmd` + `envproxy.ps1` 一个字都没动，Mac 新增的是一套并排文件，整文件夹拷贝到 Mac 就能用。
...
2. 双击 **`1-安装.command`**（首次可能要右键→打开放行一次，见 12.5）
3. 看到"监控进程已启动"即完成；**新开一个终端**验证：`env | grep -i proxy`

装完同样是三件事：开机自启（LaunchAgent）+ 立即启动监控 + 当场校正一次变量。以后任何不对劲，双击一次 `1-安装.command` 就是万能修复。

### 12.3 四个按钮（与 Windows 同语义）
...
| **4-查看状态.command** | 监控/自启动/翻墙状态/变量/最近日志 |

终端用户也可以：`bash install.sh` / `bash stop.sh` / `bash uninstall.sh`（加 `--purge` 连日志一起删）/ `bash status.sh`。`.command` 只是"双击壳"，逻辑全在 `.sh` 里。

### 12.4 文件夹里 Mac 新增了什么

```
EnvProxy/
  envproxy.sh          Mac 核心（对标 envproxy.ps1，单文件、只用系统自带命令）
  locator.sh           定位器模板（安装时复制到 ~/.envproxy/locator.sh）
  install.sh / stop.sh / uninstall.sh / status.sh
  1-安装.command … 4-查看状态.command（双击入口）
...
1. **双击没反应 / 提示"无法打开"**：这是 Gatekeeper。解法三选一：右键→打开放行一次；或终端跑 `bash install.sh`（永远有效）；或 `xattr -d com.apple.quarantine 1-安装.command` 后再双击。
2. **`chmod` 权限丢了**（从 Windows 拷过来常见）：终端进目录跑一次 `chmod +x *.command *.sh` 即可。
...
2. **core 语言是 shell 不是 python**：Mac 自带的 `/usr/bin/python3` 是 Xcode 附带的旧版本，新系统缺组件会直接报错，所以 Mac 版故意只用 `zsh/bash/curl/lsof/launchctl`（全是出厂自带），真正零依赖。**改完 `envproxy.sh` 保存为 LF、无 BOM**（与 Windows 的 UTF-8 BOM 相反，带 BOM 会顶掉 shebang 导致无法执行）。
