...
> 以后想更新版本/修复任何问题：进 `win` 文件夹，直接**再双击一次 `1-安装.cmd`** 就行，它是"万能修复按钮"，不会重复安装、不会断网、不会丢东西。

## 二、四个按钮的确切行为（小白必读）

> 四个按钮都在 `win` 文件夹里：在下面说"双击"时，都是指进 `win` 文件夹双击。
...
以后更新：`git pull` 拉到最新后，进 `win` 文件夹再双击一次 `1-安装.cmd`（万能修复：加载新代码并校正一次变量）。
...
> 四个按钮都在 `mac` 文件夹里：在下面说"双击"时，都是指进 `mac` 文件夹双击。
